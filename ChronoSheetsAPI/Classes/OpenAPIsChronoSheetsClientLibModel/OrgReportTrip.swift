//
// OrgReportTrip.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation


public struct OrgReportTrip: Codable { 


    public enum MobilePlatform: String, Codable {
        case unknown = "Unknown"
        case ios = "iOS"
        case android = "Android"
    }
    public var username: String?
    public var emailAddress: String?
    public var firstName: String?
    public var lastName: String?
    public var tripId: Int?
    public var timesheetId: Int?
    public var vehicleId: Int?
    public var userId: Int?
    public var orgId: Int?
    public var mobilePlatform: MobilePlatform?
    public var startDate: Date?
    public var endDate: Date?
    public var vehicleName: String?
    public var vehicleMake: String?
    public var vehicleModel: String?
    public var vehicleYear: String?
    public var costPerKilometer: Double?
    public var tripTotalCost: Double?
    public var totalTripDistanceMeters: Double?
    public var startAddress: String?
    public var endAddress: String?
    public var pathCoordinates: [TripCoordinate]?
    public var cacheExpiryDate: Date?

    public init(username: String?, emailAddress: String?, firstName: String?, lastName: String?, tripId: Int?, timesheetId: Int?, vehicleId: Int?, userId: Int?, orgId: Int?, mobilePlatform: MobilePlatform?, startDate: Date?, endDate: Date?, vehicleName: String?, vehicleMake: String?, vehicleModel: String?, vehicleYear: String?, costPerKilometer: Double?, tripTotalCost: Double?, totalTripDistanceMeters: Double?, startAddress: String?, endAddress: String?, pathCoordinates: [TripCoordinate]?, cacheExpiryDate: Date?) {
        self.username = username
        self.emailAddress = emailAddress
        self.firstName = firstName
        self.lastName = lastName
        self.tripId = tripId
        self.timesheetId = timesheetId
        self.vehicleId = vehicleId
        self.userId = userId
        self.orgId = orgId
        self.mobilePlatform = mobilePlatform
        self.startDate = startDate
        self.endDate = endDate
        self.vehicleName = vehicleName
        self.vehicleMake = vehicleMake
        self.vehicleModel = vehicleModel
        self.vehicleYear = vehicleYear
        self.costPerKilometer = costPerKilometer
        self.tripTotalCost = tripTotalCost
        self.totalTripDistanceMeters = totalTripDistanceMeters
        self.startAddress = startAddress
        self.endAddress = endAddress
        self.pathCoordinates = pathCoordinates
        self.cacheExpiryDate = cacheExpiryDate
    }

    public enum CodingKeys: String, CodingKey { 
        case username = "Username"
        case emailAddress = "EmailAddress"
        case firstName = "FirstName"
        case lastName = "LastName"
        case tripId = "TripId"
        case timesheetId = "TimesheetId"
        case vehicleId = "VehicleId"
        case userId = "UserId"
        case orgId = "OrgId"
        case mobilePlatform = "MobilePlatform"
        case startDate = "StartDate"
        case endDate = "EndDate"
        case vehicleName = "VehicleName"
        case vehicleMake = "VehicleMake"
        case vehicleModel = "VehicleModel"
        case vehicleYear = "VehicleYear"
        case costPerKilometer = "CostPerKilometer"
        case tripTotalCost = "TripTotalCost"
        case totalTripDistanceMeters = "TotalTripDistanceMeters"
        case startAddress = "StartAddress"
        case endAddress = "EndAddress"
        case pathCoordinates = "PathCoordinates"
        case cacheExpiryDate = "CacheExpiryDate"
    }

}
